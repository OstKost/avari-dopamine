...
- [x] Каталог содержит 150-300 товаров, 8-12 категорий после `make seed` (`FR-CATALOG-01`)
- [x] Повторный запуск seed не создаёт дублей (идемпотентность)
- [x] Поиск по названию/описанию возвращает релевантные результаты за <200мс на полном каталоге (`FR-CATALOG-02`)
- [x] Пустой поисковый запрос возвращает дефолтную постраничную выдачу (`FR-CATALOG-02`)
- [x] `ProductLookup.GetByIDs` корректно обрабатывает частично несуществующие ID (возвращает только найденные, не ошибку)
