...
- [x] Пустая корзина не может быть оформлена (`FR-ORDER-01`)
- [x] Сумма заказа всегда 10.00 RUB независимо от состава корзины (`INV-01`)
- [x] Заказ создаётся в статусе `created`, событие `order.created.v1` публикуется через outbox в той же транзакции (`FR-ORDER-01`, `NFR-REL-01`)
- [x] Невалидные переходы статуса отклоняются на уровне домена (`FR-ORDER-02`)
- [x] Все переходы статуса записаны в `order_status_history` с timestamp (`FR-ORDER-02`)
- [x] Пользователь не может иметь два `payment_pending` заказа одновременно (`INV-02`)
- [x] Заказ неизменяем после перехода в `paid` (`INV-04`) — попытка изменить состав возвращает ошибку домена
- [x] Повторная доставка Kafka-события не приводит к повторному переходу статуса (`NFR-REL-02`)
- [x] История заказов пагинируется курсором, поддерживает фильтр по статусу/дате (`FR-HISTORY-01`)
