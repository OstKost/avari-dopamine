...
- [x] Добавление/удаление/изменение количества работает корректно (`FR-CART-01`)
- [x] Корзина сохраняется между сессиями/устройствами одного пользователя (server-side, не localStorage) (`FR-CART-01`)
- [x] Смена ПВЗ не сбрасывает товары в корзине (`FR-CART-01`)
- [x] Добавление несуществующего `product_id` возвращает явную ошибку `404`, а не тихо игнорируется
