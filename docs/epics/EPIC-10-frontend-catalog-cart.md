...
- [ ] Онбординг работает с геолокацией и без неё (fallback на город) (`FR-PICKUP-01`)
- [ ] Поиск и фильтр по категории работают, обновляют URL (шарибельные ссылки на результаты поиска) (`FR-CATALOG-02`)
- [ ] Добавление в корзину — оптимистичное, без полной перезагрузки страницы (`FR-CART-01`)
- [ ] Корзина отображает фиксированную стоимость заказа 10₽ явно и заметно, независимо от состава
- [ ] Смена ПВЗ не сбрасывает товары в корзине (совпадает с backend acceptance criteria EPIC-04)
- [ ] Изображения товаров используют `next/image` с корректной конфигурацией доменов
