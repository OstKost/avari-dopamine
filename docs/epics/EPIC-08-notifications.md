...
- [x] Переподключение клиента получает текущий статус, а не только будущие события (`FR-NOTIF-01`)
- [x] Задержка от публикации в Kafka до получения клиентом < 500мс (`FR-NOTIF-01`)
- [x] Нельзя подписаться на SSE чужого заказа (403)
- [x] Соединение переживает отсутствие событий 60+ секунд (keep-alive работает, не рвётся по timeout)
